<script>
      ...
      return Swal.fire({
        ...
        preConfirm: (reasons) => {
          const data ={Comments: reasons, Approver_type: "internal", Approved: false,
            Requestor_id: requesterId, Request_id: requestId, RequesterEmail:requesterEmail}
          Post_approval_or_rejection(data)
        },
      }).then((result) => {
        if (result.isConfirmed) {
  ...
  const SendeMAIL = async (details) => {
   let email = {"Sender": "info.his@mg.kenyahmis.org","Subject": "Internal Approval Stage", "Body": details.Comments,
  ...
  <div class="rounded-t bg-white mb-0 px-6 py-6">
        ...
        <div class="text-center flex justify-between">
          <h6 class="text-blueGray-700 text-xl font-bold">RequestID: #{data.data.Request.ReqId}
            <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-indigo-600 bg-indigo-200 uppercase last:mr-0 mr-1">
            ...
            {#if data.data.Approved != null}
              <span class={`text-xs font-semibold inline-block py-1 px-2 uppercase rounded uppercase last:mr-0 mr-1" +
               " ${data.data.Approved ? 'text-green-500 bg-emerald-200' : 'text-red-900 bg-red-200'}`}>
                 {data.data.Approved ? "Approved" : "Rejected" }  <i class="fa fa-exclamation" aria-hidden="true"></i>
              </span>
            {/if}
          </h6>
        ...
        <div class="w-full lg:w-4/12">
          <div>
            ...
            <table class="items-center w-full bg-transparent border-collapse">
              ...
              <tr>
                ...
                <td><span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded bg-blueGray-200 text-indigo-600 uppercase last:mr-0 mr-1">
                  {data.data.Requester.Name}</span></td>
              </tr>
              <tr>
                <td><p class="text-blueGray-700 text-sm ">Organization :</p></td>
                <td><span
                    ...
                    {data.data.Requester.Organization}</span></td>
              </tr>
              <tr>
                <td><p class="text-blueGray-700 text-sm ">Requested :</p></td>
                <td><span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded bg-blueGray-200 text-indigo-600 uppercase last:mr-0 mr-1">
                  {moment(data.data.Created_date).format('dddd, DD MMM YYYY')}</span></td>
              </tr>
              </tbody>
            </table>
          </div>
          ...
          {#if data.data.Approver_type== 'external' }
            ...
            <table class="items-center w-full bg-transparent border-collapse">
              <tbody>
              <tr>
                <td><p class="text-blueGray-700 text-sm ">Approver :</p></td>
                <td><span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded bg-blueGray-200 text-indigo-600 uppercase last:mr-0 mr-1">
                  {data.data.Approver.Email}</span></td>
              </tr>
              <tr>
                <td><p class="text-blueGray-700 text-sm ">Approved :</p></td>
                <td><span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded bg-blueGray-200 text-indigo-600 uppercase last:mr-0 mr-1">
                  {moment(data.data.Approval_Date).format('dddd, DD MMM YYYY')}</span></td>
              </tr>
              <tr>
                <td><p class="text-blueGray-700 text-sm ">Approved :</p></td>
                <td><span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-emerald-600 bg-emerald-200 uppercase last:mr-0 mr-1">
                  {data.data.Approved} <i class="fa fa-exclamation" aria-hidden="true"></i></span></td>
              </tr>
              </tbody>
            </table>
            </div>
    ...
    <div class="flex flex-wrap">
      <div class="w-full px-4 flex-1">
        <button class="bg-indigo-700 w-full text-white active:bg-orange-600 font-bold uppercase text-xs px-4 py-2
